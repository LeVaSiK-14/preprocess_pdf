...
@measure_time
def crop_white_borders(png_lists_by_color_dir_path: str, png_lists_croped_path: str, colors: list, threshold: int = 250, margin: int = 20) -> None:
    """
    ...
    png_dirs_names = get_dirs_from_dir(png_lists_by_color_dir_path)
    for pnd_dir_name in png_dirs_names:
        png_dir_name_path = get_full_path(png_lists_by_color_dir_path, pnd_dir_name)
        
        png_dirs_names_colors = get_dirs_from_dir(png_dir_name_path)
        
        color_folders = create_dirs_by_color(png_lists_croped_path, pnd_dir_name, colors)
        
        for png_dir_name_color in png_dirs_names_colors:
            png_dir_name_color_dir_full_path = get_full_path(png_dir_name_path, png_dir_name_color)
            
            png_lists_names = get_files_from_dir(png_dir_name_color_dir_full_path)
            
            for png_list_name in png_lists_names:
                
                png_list_name_full_path = get_full_path(png_dir_name_color_dir_full_path, png_list_name)
                ...
                for color, dir_path in color_folders.items():
                    if BLUE in png_list_name:
                        cv2.imwrite(f'{color_folders.get(BLUE)}/{png_list_name}', cropped)
                    elif RED in png_list_name:
                        cv2.imwrite(f'{color_folders.get(RED)}/{png_list_name}', cropped)
                    elif PINK in png_list_name:
                        cv2.imwrite(f'{color_folders.get(PINK)}/{png_list_name}', cropped)
